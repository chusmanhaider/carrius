...
if($_POST) {
	...
	if($pass!='')
	{
		$sql ="SELECT * FROM buyer WHERE buyer_ID = {$userId}";
		if($query = $connect->query($sql))
		{
			while($result = $query->fetch_assoc())
			{
				if($pass == $result['buyer_Password'])
				{
					$updateSql = "UPDATE buyer SET 
                    buyer_FName = '$fname', 
					buyer_LName='$lname',
					...
					buyer_Email='$mail',
					buyer_CellNumber ='$cell',
					buyer_Image = '$url_one'
                    WHERE buyer_ID = {$userId} AND buyer_Password='$pass'";
